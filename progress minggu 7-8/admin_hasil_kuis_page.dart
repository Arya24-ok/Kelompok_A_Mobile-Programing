...
class AdminHasilKuisPage extends StatefulWidget { // Mendefinisikan StatefulWidget untuk halaman hasil kuis admin
  ...
  const AdminHasilKuisPage({ // Constructor
    super.key,
  ...
  @override
  State<AdminHasilKuisPage> createState() => _AdminHasilKuisPageState(); // Membuat state
}

class _AdminHasilKuisPageState extends State<AdminHasilKuisPage> { // Kelas State
  // [PERBAIKAN] Hapus inisialisasi FlutterSecureStorage
  // final _storage = const FlutterSecureStorage();

  List<HasilSiswa> _daftarHasil = []; // Variabel untuk menyimpan daftar hasil kuis dari siswa
  bool _isLoading = true; // Status loading data
  String? _error; // Variabel untuk menyimpan pesan error

  @override
  void initState() { // Dipanggil saat widget dibuat
    super.initState();
    _fetchHasilSiswa(); // Memuat data hasil kuis segera
  }

  Future<void> _fetchHasilSiswa() async { // Fungsi untuk mengambil data hasil kuis dari API
    setState(() { // Mulai proses loading
      _isLoading = true;
      _error = null;
    });

    // Ganti IP di sini
    final url = Uri.parse( // URL endpoint API untuk mengambil hasil kuis
      'http://192.168.1.17:3000/kuis/${widget.kuisId}/hasil',
    );
    ...
    try {
      ...
      if (token == null) { // Jika token tidak ada
        ...
        setState(() {
          _error = 'Error: Sesi Anda berakhir. Silakan login ulang.'; // Set pesan error otentikasi
          _isLoading = false;
        });
      ...
      if (response.statusCode == 200) { // Sukses (Status 200 OK)
      ...
      } else if (response.statusCode == 404) { // Belum ada hasil (Status 404 Not Found)
        setState(() {
          _daftarHasil = []; // Set list kosong
          _isLoading = false;
        });
      } else if (response.statusCode == 401) { // Error otentikasi (Status 401 Unauthorized)
        setState(() {
          _error = 'Error 401: Token tidak valid atau sesi habis. Silakan login ulang.'; // Set pesan error otentikasi
          _isLoading = false;
        });
      } else { // Error status code lainnya
        throw Exception(
            'Gagal memuat hasil: Status ${response.statusCode}'); // Lempar exception
      }
    } catch (e) { // Menangkap error umum (koneksi, dll.)
      if (!mounted) return;
      setState(() {
  ...
  @override
  Widget build(BuildContext context) { // Metode pembangunan UI
    return Scaffold(
      appBar: AppBar( // AppBar (kepala halaman)
        title: Text( // Judul AppBar
          'Hasil Kuis - ${widget.kuisJudul}',
          overflow: TextOverflow.ellipsis, // Jika judul terlalu panjang, tampilkan elipsis
        ),
        ...
        actions: [
          IconButton(
            ...
            tooltip: 'Refresh',
            onPressed: _fetchHasilSiswa, // Panggil fungsi ambil data saat tombol ditekan
          ),
        ],
      ),
      body: _isLoading // Konten utama halaman
          ? const Center(child: CircularProgressIndicator()) // Tampilkan loading spinner jika isLoading true
          : _error != null // Jika ada error
              ? Center( // Tampilkan pesan error
                  child: Padding(
              ...
              : _daftarHasil.isEmpty // Jika tidak loading, tidak error, dan daftar hasil kosong
                  ? const Center( // Tampilkan pesan data kosong
                      child: Text(
                        'Belum ada siswa yang mengerjakan kuis ini.',
                        style: TextStyle(fontSize: 16),
                      ),
                    )
                  : ListView.builder( // Tampilkan daftar hasil jika ada data
                      padding: const EdgeInsets.all(8.0),
                      itemCount: _daftarHasil.length, // Jumlah item dalam daftar
                      itemBuilder: (context, index) { // Fungsi pembuat item daftar
                        final hasil = _daftarHasil[index]; // Hasil siswa saat ini
                        return Card( // Menggunakan Card untuk setiap item hasil
                          margin: const EdgeInsets.only(bottom: 8.0),
                          child: ListTile( // Item daftar standar
                            leading: CircleAvatar( // Lingkaran di sebelah kiri menampilkan skor
                              backgroundColor: hasil.skor >= 75 // Warna background Avatar (Hijau jika skor >= 75, Merah jika < 75)
                                  ? Colors.green[100]
                                  : Colors.red[100],
                              foregroundColor: hasil.skor >= 75 // Warna teks Avatar
                                  ? Colors.green[800]
                                  : Colors.red[800],
                              child: Text(
                            ...
                            title: Text( // Judul ListTile (Username Siswa)
                              hasil.username,
                              style:
                                  const TextStyle(fontWeight: FontWeight.w500),
                            ),
                            subtitle: Text( // Subjudul ListTile (Tanggal Pengerjaan)

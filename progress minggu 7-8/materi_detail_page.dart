...
import 'package:flutter/foundation.dart' show kIsWeb; // [PERBAIKAN]: Import kIsWeb untuk pengecekan platform yang aman di Web.
...
class _MateriDetailPageState extends State<MateriDetailPage> {
  ...
  Future<void> _fetchMateriDetailById() async {
    ...
    final url = Uri.parse(
      ...
      'http://192.168.1.17:3000/materi/${widget.materiId}', 
    ); // URL API untuk mengambil detail materi.

    try {
      final response = await http.get(url); // Lakukan permintaan GET ke API.
      if (!mounted) return; // Keluar jika widget sudah tidak ada.
